...
long smarterSum(long n) {
    long total = 0;
    long sqrtn = sqrt(n);
    for (long divisor = 1; divisor <= sqrtn && divisor < n; divisor++) {
        if (n % divisor == 0) {
            if(divisor == 1){
                total +=1;
            }else if(divisor == sqrtn && sqrtn == sqrt(n)){
                total +=divisor;
            }else{
...
STUDENT_TEST("Add some negative numbers to isperfect"){
    EXPECT(!isPerfect((-2)));
    EXPECT(!isPerfect((-4)));
    EXPECT(!isPerfect((-5)));
    EXPECT(!isPerfect((-6)));
}
STUDENT_TEST("test smartersum compared to divisorSum"){
